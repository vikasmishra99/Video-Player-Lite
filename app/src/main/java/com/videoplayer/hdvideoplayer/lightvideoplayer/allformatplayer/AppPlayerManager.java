...
//                    mediaDataSourceFactory, extractorsFactory, null, null);
        MediaSource mediaSource = new ExtractorMediaSource.Factory(mediaDataSourceFactory).setExtractorsFactory(extractorsFactory)
                .createMediaSource(Uri.parse(AppGlobalVar.getInstance().getPath()));
    ...
    public void onFullScreen() {
        if (simpleExoPlayer == null) return;
        mAppGlobalVar.seekPosition = simpleExoPlayer.getCurrentPosition();
        mAppGlobalVar.isPlaying = simpleExoPlayer.getPlayWhenReady();
